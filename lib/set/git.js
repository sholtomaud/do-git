...
module.exports = function(settings, cb){
  ...
  ('bareGit',['testSSH'], function(remoteSetup,done){
    console.log( '--->  setting up bare git repo on ',remoteSetup.host)
...
//***
      ...
      let echo = `echo ${text} > ${remoteSetup.projectDir}/hooks/post-receive`
      debug('echo',echo)
      ...
      hook.on('close',function(error, data){
        if (!error){
          ...
          const chMod = spawn('ssh', [remoteSetup.host, '-p', remoteSetup.port, chmod])
          chMod.on('close',function(error, data){
            debug('kgo task: create post-commit git hook for: ', remoteSetup.projectName )
            console.log( '---> created post-commit git hook')
            done( null, remoteSetup )
          })
        }
        else{
          console.log('problem in creating git hook')
          done(error,'probs')
        }
      })
    })
  ...
  ('mkdir',['bareGit','testSSH'],function(bareGit, remoteSetup, done){
    ...
    rm.on('close',function(error, data){
      ...
      console.log( `---> removed working Git directory ${remoteSetup.gitWorkTree}`)
      const mk = spawn('ssh', [remoteSetup.host, '-p', remoteSetup.port, mkDir])
