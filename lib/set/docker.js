...
CMD [ \\"npm\\", \\"start\\" ]"`
      ...
      let writeYML = `FROM node:${lastRemote.nodeVersion}\n\n\
ENV HOME=/usr/src/apps/\n\
RUN mkdir -p \$HOME\n\
WORKDIR \$HOME\n\
COPY package.json \$HOME\n\n\
...
// RUN npm install --global npm@${lastRemote.npmVersion}\n\n\
      ...
      fs.writeFile('./Dockerfile', writeYML, 'utf8', function(error, data){
        if (error) console.log('Oops! Problem creagin Dockerfile', error)
        debug('kgo task: added Dockerfile')
        console.log( '---> created Dockerfile')
        done(error,'done')
...
services: \n\
  ${packg.name}: \n\
    ...
    ports: \n\
      ...
      fs.writeFile('./docker-compose.yml', writeComposeYML, 'utf8', function(error, data){
        if (error) console.log('Opps!. Problem creating Dockerfile', error)
        debug('kgo task: added Dockerfile')
        console.log( '---> created docker-compose.yml')
        done(error,'done')
