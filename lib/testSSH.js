...
module.exports = function( settings, cb){
  kgo('lastPackageSuccess',function(done){
    db.get('lastSSHSuccess', function (err, lastSSHSuccess) {
      if (err ){
        debug('db problem with get: ', err)
        console.warn(colors.magenta('---> db problem'), err, 'using defaults')
        done(null,{})
      }
      else {
        ...
        if ( last && last.hasOwnProperty(settings.pack.name) ){
          ...
          console.log('---> Nothing in db. Setting last to {}')
          done(null, {})
        }
      }
    })
  ...
  ('test',['lastPackageSuccess'],function(last, done){
    if (last &&  last.hasOwnProperty('success') ){
      if ( last.success.time + timeout > Date.now() ) {
        ...
        const testSSH = spawn('ssh', [ '-i',last.success.i,last.success.host ,'-p',last.success.port, lastTouch] )
  ...
  ('testAllRemotes',['getAllRemotes','lastPackageSuccess'],function(remotes,last,done){
    ...
    function callSSH(i, comms){
      ...
      let projectGitWorkTree = `${remote.gitWorkTree}/${settings.pack.name}`
      let sshSuccess = {}

      sshSuccess.loginID = remote.loginID
      sshSuccess.host = userHost
      sshSuccess.port = port
      sshSuccess.gitWorkTree = projectGitWorkTree
      ...
      sshSuccess.i = path.join(HOME,'/.ssh/cloud.key')

      const testSSH = spawn('ssh', [sshSuccess.host ,'-p',sshSuccess.port,'-i', sshSuccess.i, testTouch],{stdio:'inherit'} )

      testSSH.on('close', function(error, data){
        if (error){
          console.log('--->',colors.red('unable to connect to'), sshSuccess.i, sshSuccess.host, sshSuccess.port );
