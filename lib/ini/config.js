...
module.exports = function(settings, npmInit, cb){
  ...
  const systemQuestions = require(  path.join(settings._dogitdir,'/lib/ini/systemQuestions.js') )
  ...
  kgo('archive',function(done){
    ...
    db.get(settings.pack.name, function ( err, archive ) {
      if (err || !archive ){
        ...
        done(null, require( path.join(settings._dogitdir,'/lib/ini/defaults.json') ) )
      }
      else{
